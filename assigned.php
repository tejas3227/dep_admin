...
if (isset($_GET['PRN'])) {
    $PRN = $_GET['PRN'];
    $delete = mysqli_query($conn, "DELETE FROM assign WHERE PRN = '$PRN'");
    if ($delete) {
        echo "<script>alert('PRN " . $PRN . " has been removed successfully for Reassignment.');</script>";
    } else {
...
<body data-layout="horizontal" data-topbar="dark">
  ...
  <h4 class="card-title mx-auto" style="font-size: 22px;">Assigned Students to Mentors</h4>
</div>
<!-- end card header -->
                                <div class="card-content table-responsive">
                                    <table class="table table-bordered border-dark" id="table1">
                                        <div style="text-align: center;">
                                            <a
                                                href="assigned_report.php?dept=<?php echo $dept; ?>&year=<?php echo $year; ?>">Download
                                                PDF File</a>
                                        </div>
                                        <?php
                                        $result = mysqli_query($conn, "SELECT * FROM assign WHERE `Department` = '$dept' AND `Class` = '$year' ORDER BY TID");
                ...
                <a href='assigned.php?PRN=" . $row['PRN'] . "&dept=" . $dept . "&year=" . $year . "&AY=" . $AY . "' class='btn1'>
