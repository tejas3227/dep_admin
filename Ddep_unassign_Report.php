...
if (isset($_GET['dept']) && !empty($_GET['dept'])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM studinfo WHERE `Department` = '$dept' AND `PRN` NOT IN (SELECT `PRN` FROM assign WHERE `Department` = '$dept') ORDER BY `Class`, `Name`");
    $count = mysqli_num_rows($result);
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 10, "Total Unassigned Students: $count", 0, 1, 'C');
    ...
    // Output the PDF
    $pdf->Output($dept . '_Unassignment_Report.pdf', 'D');
